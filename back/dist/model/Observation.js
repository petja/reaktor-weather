...
function getCities() {
    return knex.select().from('cities').then(cities => {
...
function getList(cities, page) {
    return knex.select().from('observations').column(['observations.id', 'cities.name as city', 'temperature', 'timestamp']).join('cities', 'cities.id', 'observations.city').whereIn('city', cities).orderBy('timestamp', 'desc').limit(ITEMS_PER_PAGE).offset(ITEMS_PER_PAGE * page);
}

function getSummary() {
    // Get latest temperature of each city
    // LEFT JOIN ensures also cities with no data will be returned
    const latest = knex.select().from('cities').column(['cities.id AS city', 'cities.name AS name', 'timestamp', 'temperature']).leftJoin('observations', 'observations.city', 'cities.id').whereIn('timestamp', function () {
        this.select().from('observations').max('timestamp').groupBy('city');
    }).orWhereNull('timestamp');

    // Get min and max temperatures of each city
    const minMax = knex.select().from('observations').column(['city']).min('temperature as min').max('temperature as max').whereRaw('timestamp > DATE_SUB(NOW(), INTERVAL 1 DAY)').groupBy('city');

    // Run queries
    return Promise.all([latest, minMax]).then(resp => {
